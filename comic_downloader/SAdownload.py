...
import aiohttp
import requests
from .modules.base_downloader import BaseDownloader
...
class Downloader(BaseDownloader):
    ...
    def _comic_file_page_link(self, page: int|str) -> str:
        return f"{self._comic_main_page_link}?s={page}"

    def _comic_file_link(self, page: int|str) -> str:
        return f"{self._comic_domain}/{self._comic_page_title(page)}_small.jpg"

    def _comic_filename(self, page: int|str) -> str:
        """Получение имени файла страницы комикса"""
        file_link = self._comic_file_link(page)
        return file_link.rsplit("/", 1)[-1]

    def _comic_page_title(self, page: int|str) -> str:
        return f"SA_{page:0>4}"
    ...
    def _findlast_check(self, page: int|str, session: requests.Session|None=None) -> int:
        ...
        if session:
            ...
            _get = requests.get
        with _get(self._comic_file_link(page)) as response:
    ...
    async def async_download_comic_page(
        ...
        if not session:
            return self.download_comic_page(page)

        # Путь к скачанному файлу
        comic_filepath = os.path.join(self.folder, self._comic_filename(page))
        # Перескачивать уже существующий файл не нужно
        if not self._check_corrects_file(comic_filepath):
            async with session.get(self._comic_file_link(page)) as resp:
    ...
    def downloadcomic(self) -> int:
        ...
        for num in range(self.first, self.last):
            if (result := self.download_comic_page(num)) and last_success == result:
    ...
    async def async_downloadcomic(self) -> int:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            tasks = [
                self.async_download_comic_page(i, session=session)
